	...
	<view class="fix-top-window">
		<view class="uni-header">
			...
			<view class="uni-group">
				...
				<!-- #ifdef H5 -->
				<download-excel class="hide-on-phone" :data="dataList" :type="exportExcel.type" :name="exportExcel.filename">
		...
		<view class="uni-container">
			...
			>
				<uni-table ref="table" :loading="loading" border stripe :type="multiSelect ? 'selection' : ''" :emptyText="error.message || '没有更多数据'" @selection-change="selectionChange">
					<uni-tr>
						<uni-th @sort-change="sortChange" :sortable="sortable" filter-type="filterType" v-for="(field, thindex) in fields" :key="thindex">
							{{ field }}
						</uni-th>
						<uni-th>删除</uni-th>
					</uni-tr>
					<uni-tr v-for="(row, index) in computedData" :key="index">
						<uni-td v-for="(field, fieldIndex) in row" :key="fieldIndex">
							...
							<!-- switchBool-->
							<switch v-else-if="field.type === 'switchBool'" :checked="field.val === 'true'" style="transform: scale(0.5)" @change="editOne($event.target.value, index, field.key)" />
						</uni-td>
				...
				<view class="uni-pagination-box">
					<picker class="select-picker" mode="selector" :value="pageSizeIndex" :range="pageSizeOption" @change="changeSize">
